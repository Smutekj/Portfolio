...
void main(void) {
	...
	vec2 uv= (v_tex_coord - u_bar_min_uv)/u_bar_size_uv;
	vec2 st = uv;
	...
	float inside = n;
    float border = n2-n;
    vec3 inside_c = vec3(0.7647, 0.0, 1.0);
    vec3 border_c = vec3(0.0, 1.0, 0.298);
	
	float health_ratio = v_color.a;
    
	float health_min_uv = 25. /220.;
	float health_max_uv = 195. /220.;
	float health_alpha = 1.-smoothstep(health_ratio, health_ratio + 0.03, (uv.x - health_min_uv) / (health_max_uv - health_min_uv));
	
	vec3 res = mix(inside_c, border_c, border);
	res = mix(vec3(0.), res, inside_region * health_alpha);
	res += vec3(0,0,1) * texel.a * texel.r; //! add blur 
	res = mix(res, vec3(0.0, 1.0, 0.4157), outline_region);
	FragColor = vec4(res * texel.a, texel.a); //health_alpha * n2*mix(0.5, 1., border));
	// FragColor = vec4(mix(inside_c, border_c, border), 1.); //health_alpha * n2*mix(0.5, 1., border));
