...
void main()
{
    ...
    float outline= smoothstep(-0.18, -0.06, sdf) - smoothstep(-0.04, 0.0, sdf);
    // float outline = smoothstep(v_blur_uv, -v_blur_uv, sdf) 
    //               - smoothstep(v_blur_uv, -v_blur_uv, sdf + 0.*v_outline_width);
    
    vec4 result = vec4(0.5*v_fill_color.rgb * v_fill_color.a * shape, shape * v_fill_color.a);
    result = mix(result, 10.*v_outline_color, outline);
