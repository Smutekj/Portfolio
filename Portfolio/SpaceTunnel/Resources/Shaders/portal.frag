...
void main() {
    ...
    float speed = 0.9f / v_duration;

    float angle = -1.0f * log2(length(uv));
    ...
    float circle_radius = speed * v_time + 0.05f * radiusNoise + 0.03f * radiusNoiseFast; // perturb the portal
    ...
    color = mix(color, complementary(v_color.rgb), circle_inside_region);
