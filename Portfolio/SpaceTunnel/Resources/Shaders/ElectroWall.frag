...
uniform float u_scale_x = 2.5;
uniform float u_thickness = 0.03;
uniform float u_speed = 3.0;
uniform float u_glow_thickness = 0.04;
uniform vec4 u_glow_color = vec4(0.0, 1.0, 0.2, 1.0);
...
{          
	...
	for ( int i = 0; i < lightning_number/2; i++){
		t = uv*u_amplitude  + (vec2(float(i), -float(i)) - u_time*u_speed);
		y = fbm(t);
